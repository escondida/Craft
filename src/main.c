...
void
interpolate_player(Player *player)
{
	State *s1 = &player->state1;
	State *s2 = &player->state2;
	float t1 = s2->t - s1->t;
	...
	float p = MIN(t2 / t1, 1);
	update_player(
			player, s1->x + (s2->x - s1->x) * p, s1->y + (s2->y - s1->y) * p,
...
float
player_player_distance(Player *p1, Player *p2)
{
	State *s1 = &p1->state;
	State *s2 = &p2->state;
	float x = s2->x - s1->x;
...
float
player_crosshair_distance(Player *p1, Player *p2)
{
	State *s1 = &p1->state;
	State *s2 = &p2->state;
...
Player *
player_crosshair(Player *player)
{
	Player *result = 0;
...
int
chunk_distance(Chunk *chunk, int p, int q)
{
...
int
chunk_visible(float planes[6][4], int p, int q, int miny, int maxy)
{
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < 8; j++) {
			float d = planes[i][0] * points[j][0] +
					planes[i][1] * points[j][1] + planes[i][2] * points[j][2] +
					planes[i][3];
			if (d < 0) {
				out++;
			} else {
...
int
hit_test(
		int previous, float x, float y, float z, float rx, float ry, int *bx,
		int *by, int *bz)
{
	int result = 0;
	float best = 0;
...
int
hit_test_face(Player *player, int *x, int *y, int *z, int *face)
{
	State *s = &player->state;
...
int
collide(int height, float *x, float *y, float *z)
{
	int result = 0;
	int p = chunked(*x);
	int q = chunked(*z);
...
void
light_fill(char *opaque, char *light, int x, int y, int z, int w, int force)
{
...
compute_chunk(WorkerItem *item)
{
	...
	MAP_FOR_EACH(map, ex, ey, ez, ew)
	{
		...
		if (is_plant(ew)) {
			...
			make_plant(
					data + offset, min_ao, max_light, ex, ey, ez, 0.5, ew,
					rotation);
		} else {
			make_cube(
					data + offset, ao, light, f1, f2, f3, f4, f5, f6, ex, ey,
					ez, 0.5, ew);
		}
		offset += total * 60;
	}
...
void
generate_chunk(Chunk *chunk, WorkerItem *item)
{
...
void
map_set_func(int x, int y, int z, int w, void *arg)
{
	Map *map = (Map *)arg;
	map_set(map, x, y, z, w);
}
...
void
init_chunk(Chunk *chunk, int p, int q)
{
	chunk->p = p;
	chunk->q = q;
	chunk->faces = 0;
...
void
create_chunk(Chunk *chunk, int p, int q)
{
...
void
ensure_chunks_worker(Player *player, Worker *worker)
{
	State *s = &player->state;
	float matrix[16];
	set_matrix_3d(
...
void
unset_sign_face(int x, int y, int z, int face)
{
	int p = chunked(x);
	int q = chunked(z);
	Chunk *chunk = find_chunk(p, q);
...
void
set_sign(int x, int y, int z, int face, const char *text)
{
	int p = chunked(x);
	int q = chunked(z);
...
void
toggle_light(int x, int y, int z)
{
	int p = chunked(x);
	int q = chunked(z);
	Chunk *chunk = find_chunk(p, q);
...
void
set_light(int p, int q, int x, int y, int z, int w)
{
	Chunk *chunk = find_chunk(p, q);
	if (chunk) {
		Map *map = &chunk->lights;
...
void
_set_block(int p, int q, int x, int y, int z, int w, int dirty)
{
	Chunk *chunk = find_chunk(p, q);
	if (chunk) {
		Map *map = &chunk->map;
...
void
set_block(int x, int y, int z, int w)
{
	int p = chunked(x);
	int q = chunked(z);
...
void
record_block(int x, int y, int z, int w)
{
...
void
builder_block(int x, int y, int z, int w)
{
	if (y <= 0 || y >= 256) {
		return;
	}
...
int
render_chunks(Attrib *attrib, Player *player)
{
	int result = 0;
	State *s = &player->state;
...
void
render_signs(Attrib *attrib, Player *player)
{
	State *s = &player->state;
	int p = chunked(s->x);
	int q = chunked(s->z);
...
void
render_sign(Attrib *attrib, Player *player)
{
...
void
render_players(Attrib *attrib, Player *player)
{
	State *s = &player->state;
	float matrix[16];
	set_matrix_3d(
...
void
render_sky(Attrib *attrib, Player *player, GLuint buffer)
{
	State *s = &player->state;
	float matrix[16];
	set_matrix_3d(
...
void
render_wireframe(Attrib *attrib, Player *player)
{
	State *s = &player->state;
	float matrix[16];
	set_matrix_3d(
...
void
render_crosshairs(Attrib *attrib)
{
	float matrix[16];
...
void
render_text(Attrib *attrib, int justify, float x, float y, float n, char *text)
{
	float matrix[16];
	set_matrix_2d(matrix, g->width, g->height);
	glUseProgram(attrib->program);
...
void
array(Block *b1, Block *b2, int xc, int yc, int zc)
{
	if (b1->w != b2->w) {
		return;
	}
...
void
cube(Block *b1, Block *b2, int fill)
{
	if (b1->w != b2->w) {
		return;
	}
...
void
sphere(Block *center, int radius, int fill, int fx, int fy, int fz)
{
...
void
cylinder(Block *b1, Block *b2, int radius, int fill)
{
	if (b1->w != b2->w) {
		return;
	}
...
void
parse_command(const char *buffer, int forward)
{
	char username[128] = {0};
	char token[128] = {0};
	char server_addr[MAX_ADDR_LENGTH];
	...
	if (sscanf(buffer, "/identity %128s %128s", username, token) == 2) {
		...
		if (db_auth_select(username)) {
			...
			sscanf(buffer, "/online %128s %d", server_addr, &server_port) >=
			1) {
		g->mode_changed = 1;
		g->mode = MODE_ONLINE;
		...
		snprintf(
				g->db_path, MAX_PATH_LENGTH, "cache.%s.%d.db", g->server_addr,
				g->server_port);
...
void
on_char(GLFWwindow *window, unsigned int u)
{
	if (g->suppress_char) {
		g->suppress_char = 0;
		return;
...
void
on_scroll(GLFWwindow *window, double xdelta, double ydelta)
{
	...
	while (running) {
		...
		while (1) {
			...
			if (SHOW_CHAT_TEXT) {
				for (int i = 0; i < MAX_MESSAGES; i++) {
					...
					if (strlen(g->messages[index])) {
						render_text(
								&text_attrib, ALIGN_LEFT, tx, ty, ts,
								g->messages[index]);
						ty -= ts * 2;
					}
				}
			...
			if (SHOW_PLAYER_NAMES) {
				if (player != me) {
					render_text(
							&text_attrib, ALIGN_CENTER, g->width / 2, ts, ts,
							player->name);
				}
				Player *other = player_crosshair(player);
				if (other) {
					render_text(
							&text_attrib, ALIGN_CENTER, g->width / 2,
			...
			if (g->observe2) {
				...
				if (SHOW_PLAYER_NAMES) {
					render_text(
							&text_attrib, ALIGN_CENTER, pw / 2, ts, ts,
							player->name);
				}
			}
